...
public class Menu_GameWon : MonoBehaviour
{
    ...
    public GameObject gameWonCreds;
    public GameObject gameWonHomeButton;
    public AudioClip soundCouting;
    [Range(0, 1)]
    public float soundCoutingVolume = 0.5f;
    public AudioClip gameWonCreditsMusic;
    ...
    public void gameWonCredits()
    {
        gameWonCreds.SetActive(true);
        StartCoroutine(WaitingforCredits(5.0f));

        SoundManager.PlayMusic(gameWonCreditsMusic);

    }

    IEnumerator WaitingforCredits(float time)
    {
        yield return new WaitForSeconds(time);
        gameWonHomeButton.SetActive(true);
    }
}
